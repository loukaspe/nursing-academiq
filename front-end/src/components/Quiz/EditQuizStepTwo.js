...
const EditQuizStepTwo = () => {
    ...
    const handleQuestionCheckboxChange = (question) => {
        setQuiz(prevQuiz => {
            ...
            const updatedQuestions = prevSelected.some(q => q.ID === question.ID)
                ? prevSelected.filter(q => q.ID !== question.ID) // remove
                : [...prevSelected, question]; // add
        ...
        <React.Fragment>
            ...
            <div className="questionsSelectChaptersButtonContainer">
                ...
                <div className="questionsSelectChaptersRightButtons">
                    <Link className="questionsSelectChaptersSaveButton" to={`/courses/${quiz.course.ID}/quizzes/${quiz.id}/edit`}>Προηγούμενο</Link>
                    <Link className="questionsSelectChaptersSaveButton" to={`/courses/${quiz.course.ID}/quizzes/${quiz.id}/edit/step-three`}>Επόμενο</Link>
                </div>
            </div>
        </React.Fragment>
    );
...
export default EditQuizStepTwo;